...
def stop_resource(provider_credentials: dict, instance_id: str, db, user_id: int) -> str:
    provider = provider_credentials.get("provider", "aws").lower()
    status = "success"
    details = ""
    ...
    try:
        if provider != "aws":
            raise Exception("Arrêt supporté uniquement pour AWS dans ce workflow.")

        logger.info(f" [AWS] Arrêt de l'instance {instance_id}")
        ec2 = boto3.client(
            "ec2",
            aws_access_key_id=provider_credentials["AWS_ACCESS_KEY_ID"],
            aws_secret_access_key=provider_credentials["AWS_SECRET_ACCESS_KEY"],
        ...
        details = f"Instance {instance_id} AWS en cours d'arrêt."
        logger.info(f" [AWS] {details}")
        return details

    except Exception as e:
        status = "failure"
        details = str(e)
        logger.info(f" [{provider.upper()}] stop_resource failed: {details}")
        raise
...
def start_resource(provider_credentials: dict, instance_id: str, db, user_id: int) -> str:
    provider = provider_credentials.get("provider", "aws").lower()
    status = "success"
    details = ""
    ...
    try:
        if provider != "aws":
            raise Exception("Démarrage supporté uniquement pour AWS dans ce workflow.")

        logger.info(f" [AWS] Démarrage de l'instance {instance_id}")
        ec2 = boto3.client(
            "ec2",
            aws_access_key_id=provider_credentials["AWS_ACCESS_KEY_ID"],
            aws_secret_access_key=provider_credentials["AWS_SECRET_ACCESS_KEY"],
        ...
        details = f"Instance {instance_id} AWS en cours de démarrage."
        logger.info(f" [AWS] {details}")
        return details
        ...
        status = "failure"
        details = str(e)
        logger.info(f" [{provider.upper()}] start_resource failed: {details}")
        raise
